...
df_all <- read.csv("./input/AFG2002_JMMI_R52_Oct24_recoded.csv")
...
# a function for re-coding availability responses for each record
...
district_median_prices <- calculate_median_prices(df, all_of(item_prices))
national_median_prices <- calculate_national_median_prices(df, all_of(item_prices))
...
district_price_modified <- district_price_modified %>%
  mutate(price_modifiers = rowSums(select(., ends_with("_point")), na.rm = TRUE)) %>%
  select(afg_dist_code, price_modifiers)
...
affordability_info <- financial_info %>%
  ...
  mutate(
    affordability_mark = case_when(
      financial_access_mark + price_modifiers <= 9 & financial_access_mark + price_modifiers >= 0 ~ financial_access_mark + price_modifiers,
      financial_access_mark + price_modifiers > 9 ~ 9,
      financial_access_mark + price_modifiers < 0 ~ 0 
    ),
...
infrastructure_info <- district_damaged_facilities_df %>%
  ...
  mutate(across(where(is.numeric), round, 1)) %>% 
  select(afg_dist_code, survey_count , everything(), -c(modality_sum, modality_percetage))
}
...
mfs <- mfs %>%
  mutate(
    ...
    classification = case_when(
      MFS >= 80 & low_dimension == 0 ~ "Full functionality",
      MFS >= 60 | low_dimension <= 1 ~ "Limited functionality",
      (MFS >= 25 & MFS < 60) | low_dimension <= 2 ~ "Poor functionality",
      MFS < 25 | low_dimension >= 3 ~ "Severe issues",
...
dashboard_mfs <- merged_mfs %>% mutate(mfi_map = case_when(
  ...
  classification == "Severe issues" ~ "Severe issues",
