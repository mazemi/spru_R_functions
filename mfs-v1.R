...
# import clean data:
df_all <- read.csv("./input/AFG2002_JMMI_R33_MAR23_recoded.csv")

# importing previous round food basket data:
previous_food_basket <- read_xlsx("./input/District_Median_JMMI_R32.xlsx", sheet = 5) %>% 
  select(afg_district, previous_food_basket = food_basket_AFN)

# districts with less than four records:
small_data_districts <- df_all %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ) %>% 
...
df <- df_all %>% filter(!(afg_dist %in% small_data_districts$afg_dist))

# winterization constant:
include_winterization <- TRUE
...
items_availability_df <- df %>% select(afg_dist, all_of(items_available_marketplace))
...
items_availability_df <- items_availability_df %>% mutate(
 ...
 tomatoes_availability_score = case_when(
  ...
  safe_water_availability_score = case_when(
    items_available_marketplace_safe_water == "Widely_available" ~ AVALABILITY.CONFIG$safe_water_widely_available,
    items_available_marketplace_safe_water == "Limited_availability" ~ AVALABILITY.CONFIG$safe_water_limited_available,
    items_available_marketplace_safe_water == "Completely_unavailable" ~ 0,
    TRUE ~ NA_real_ 
  ),
  ...
  coal_availability_score = case_when(
    items_available_marketplace_coal == "Widely_available" ~ AVALABILITY.CONFIG$coal_widely_available,
    items_available_marketplace_coal == "Limited_availability" ~ AVALABILITY.CONFIG$coal_limited_available,
    items_available_marketplace_coal == "Completely_unavailable" ~ 0, 
    TRUE ~ NA_real_
  ),
...
district_items_availability_df <- items_availability_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count= n(),
    # mode of food items:
...
district_items_availability_df <- district_items_availability_df %>% rowwise() %>%
  ...
  ungroup() %>% 
  select(afg_dist, survey_count, wheat_availibility, everything(), - c(wheat_local_availability, wheat_imported_availability))

# final calculation of availability index:
avalibibity_info_full <- district_items_availability_df %>% 
  mutate(
    avalibibity_score = case_when(
      include_winterization == TRUE ~ rowSums(.[3:26], na.rm=TRUE),
      TRUE ~ rowSums(.[3:22], na.rm=TRUE)
    ),
    avalibibity_index = avalibibity_score / ifelse(include_winterization, 1.8, 1.47)  # mapping score to maximum 30 (convert scores to index)
  )

avalibibity_info <- avalibibity_info_full %>%select(afg_dist, avalibibity_index)

# 2 Affordability index ***********************************

# 2.1 Price point modifier [Food and NFIs]:
food_nfi_item_prices <- c("wheat_local_calculation",
                          "wheat_imported_calculation",
...
winterization_item_prices <- c("firewood_calculation",
                               "coal_calculation",
                               "blanket_price",
                               "winter_jacket_price")

# median prices in districts:
district_food_nfi_prices_df <- df %>% select(afg_dist, all_of(food_nfi_item_prices)) %>% 
  group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
    wheat_local_median = median(as.numeric(wheat_local_calculation), na.rm = TRUE),
    wheat_imported_median = median(as.numeric(wheat_imported_calculation), na.rm = TRUE),
    rice_median = median(as.numeric(rice_price_calculation), na.rm = TRUE),
    veg_oil_median = median(as.numeric(veg_oil_calculation), na.rm = TRUE),
    pulses_lentils_median = median(as.numeric(pulses_lentils_calculation), na.rm = TRUE),
    pulses_beans_median = median(as.numeric(pulses_beans_calculation), na.rm = TRUE),
    pulses_split_peas_median = median(as.numeric(pulses_split_peas_calculation), na.rm = TRUE),
    salt_median = median(as.numeric(salt_calculation), na.rm = TRUE),
    sugar_median = median(as.numeric(sugar_price), na.rm = TRUE),
    tomatoes_median = median(as.numeric(tomatoes_price), na.rm = TRUE),
    sanitary_pad_median = median(as.numeric(sanitary_pad_calculation), na.rm = TRUE),
...
national_prices_df <- df %>% select(all_of(food_nfi_item_prices), all_of(winterization_item_prices)) %>% 
  summarise(
    ...
    cooking_pot_median = median(as.numeric(cooking_pot_price), na.rm = TRUE),
    water_container_median = median(as.numeric(water_container_price), na.rm = TRUE),
    firewood_median = median(as.numeric(firewood_calculation), na.rm = TRUE),
    coal_median = median(as.numeric(coal_calculation), na.rm = TRUE),
...
price_point_modifier <- function(item_median){
  ...
  result = case_when(
    ...
    {{item_median}} <= national_prices_df[[item]] * 2 ~ -2,
    TRUE ~ NA_real_
  )
  return (result)
}
...
district_food_nfi_prices_df <- district_food_nfi_prices_df %>% 
  mutate(
    ...
    rice_point = price_point_modifier(rice_median),
...
district_food_nfi_prices_df$price_count <- rowSums(!is.na(district_food_nfi_prices_df[, 3:25]))

# sum of all the points in every districts:
food_nfi_points <- district_food_nfi_prices_df %>%
  mutate(
    food_nfi_point = rowSums(select(., ends_with("point")) , na.rm = TRUE)
  ) %>% 
  select(afg_dist, food_nfi_point)

# 2.2 Price point modifier [Winterization]:
# median of prices in districts:
district_winterization_prices_df <- df %>% select(afg_dist, all_of(winterization_item_prices)) %>% 
  group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
    firewood_median = median(as.numeric(firewood_calculation), na.rm = TRUE),
...
winterization_points <- district_winterization_prices_df %>%
  ...
  select(afg_dist, winterization_point)

# 2.3 Financial Access:
financial_df <- df %>% select(afg_dist, 
...
financial_info <- financial_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  ) %>% select(afg_dist, financial_access_score)

# 2.4 Indebtedness change:
indebtedness_df <- df %>% select(afg_dist, purchase_credit)

indebtedness_info <- indebtedness_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  ) %>% select(afg_dist, indebtedness_problem_score)

# 2.5 Food basket stability:

# required data for calculation of the food basket:
food_basket_df <- df %>% select(afg_dist, wheat_imported_calculation, rice_price_calculation,
...
food_basket_df <- food_basket_df %>%
  rowwise() %>% 
  mutate(pulses_min_calculation = min(pulses_lentils_calculation, pulses_beans_calculation, pulses_split_peas_calculation)) %>% 
  ungroup() %>% 
...
food_basket_district_df <- food_basket_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  select(afg_dist, survey_count, food_basket)

# merging current food basket with previous one:
food_basket_stability_df <- food_basket_district_df %>% 
  left_join(previous_food_basket, by= c("afg_dist"="afg_district"))
...
food_basket_stability_df <- food_basket_stability_df %>% mutate(
  food_basket_stability_score = case_when(
    abs(((food_basket - previous_food_basket)/((food_basket + previous_food_basket)/2)) * 100) <= 5 ~ 5,
    abs(((food_basket - previous_food_basket)/((food_basket + previous_food_basket)/2)) * 100) > 5 & 
      abs(((food_basket - previous_food_basket)/((food_basket + previous_food_basket)/2)) * 100) <= 10 ~ 3,
    abs(((food_basket - previous_food_basket)/((food_basket + previous_food_basket)/2)) * 100) > 10 ~ 0,
    TRUE ~ NA_real_
  )
)

# final affordability index:
afordibility_info <- financial_info %>% 
  inner_join(indebtedness_info, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(food_basket_stability_df, by= c("afg_dist"="afg_dist")) %>%
  inner_join(food_nfi_points, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(winterization_points, by= c("afg_dist"="afg_dist")) %>% mutate(
    max_score = case_when(
      include_winterization == TRUE & !is.na(food_basket) & !is.na(previous_food_basket) ~ 27,
      include_winterization == TRUE & (is.na(food_basket) | is.na(previous_food_basket)) ~ 22,
      include_winterization == FALSE & !is.na(food_basket) | !is.na(previous_food_basket) ~ 19,
      TRUE ~ 14
    ),
    afordibility_score = case_when(
    ...
    afordibility_final_score = case_when(
      afordibility_score >= 0 & afordibility_score <= max_score ~ afordibility_score, 
      afordibility_score < 0 ~ 0,
      afordibility_score > max_score ~ max_score
    ),
    
    # map final score to 15
    ### Comment from PV: I would really suggest to do this as a caculation i.e. not hardcoding the multiplication factor
    afordibility_index = case_when(
      max_score == 14 ~ afordibility_final_score * 1.071428571,
      max_score == 19 ~ afordibility_final_score * 0.789473684,
      max_score == 22 ~ afordibility_final_score * 0.681818182,
      max_score == 27 ~ afordibility_final_score * 0.555555556,
    )
  )
...
all_supply_chain_df <- df %>% select(afg_dist, "X_uuid") %>% 
  left_join(supply_chain_disruption_df, by=c("X_uuid"="X_uuid")) %>% 
  select(afg_dist, disruption)

# supply chain score for all districts:
district_supply_chain_df <- all_supply_chain_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  ) %>% select(afg_dist, disruption_score)
  
# 3.2 Supplier Diversity for Food 
district_one_food_supplier_df <- df %>% select(afg_dist, food_supplier_count) %>% 
group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  select(afg_dist, one_food_supplier_score)

# 3.3 Supplier Diversity for NFI 
district_one_nfi_supplier_df <- df %>% select(afg_dist, nfi_supplier_count) %>% 
  group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  select(afg_dist, one_nfi_supplier_score)

# 3.4 Restocking [Food and NFIs]
restock_df <- df %>% select(afg_dist,
...
district_restock_df <- restock_df %>% group_by(afg_dist) %>% 
  summarise(
...
) %>% select(afg_dist, restock_score)

# 3.5 Restocking [Winterization]
winterization_restock_df <- df %>% select(afg_dist, 
                                          diff_stock_restock_firewood,
                                          diff_stock_restock_coal,
                                          diff_stock_restock_blanket,
                                          diff_stock_restock_winter_jacket
                                        )
district_winterization_restock_df <- winterization_restock_df %>% group_by(afg_dist) %>% 
  summarise(
...
) %>% select(afg_dist, winterization_restock_score)

# final Resilience index:
resilience_info <- district_supply_chain_df %>% 
  inner_join(district_one_food_supplier_df, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_one_nfi_supplier_df, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_restock_df2, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_winterization_restock_df2, by= c("afg_dist"="afg_dist"))
...
resilience_info <- resilience_info %>% mutate(
  ...
  # Comment from PV: Again here I would advice that we do the calculation in the code rather than hardcode the factor...
...
damaged_facilities_df <- df %>% select(afg_dist, physical_access.hazardous_building)
district_damaged_facilities_df <- damaged_facilities_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
  ...
  select(afg_dist, damaged_facilities_score)

# 4.2 Storage
storage_df <- df %>% select(afg_dist, storage)
storage_df <- storage_df %>% mutate(
  no_storage = if_else(storage == "yes_my_own" | storage == "yes_elsewhere", 0, 1) 
)

district_storage_df <- storage_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(), 
  ...
  select(afg_dist, no_storage_score)

# 4.3 Financial Services
financial_services_df <- df %>% select(afg_dist,
...
district_financial_services_df <- financial_services_df %>% group_by(afg_dist) %>% 
  summarise(
    n(),
...
) %>% select(afg_dist, fsp_types_score)

# 4.4 Payment Moralities
payment_df <- df %>% select(afg_dist, 
...
district_payment_df <- payment_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
...
infrastructure_info <- district_damaged_facilities_df %>% 
  inner_join(district_storage_df, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_financial_services_df2, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_payment_df, by= c("afg_dist"="afg_dist"))

infrastructure_info$infrastructure_index <- infrastructure_info$damaged_facilities_score +
...
# 5.1 Physical access
physical_access_df <- df %>% select(afg_dist, 
                                     ...
                                     physical_access.other
                                      ...
                                      physical_access_issue = case_when(
                                        ...
                                        physical_access.hazards_roads == 1 |
                                    ...
                                    ) %>% select(afg_dist, physical_access_issue)



district_physical_access_df <- physical_access_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
...
hazards_roads_df <- df %>% 
  select(afg_dist, physical_access.hazards_roads)

district_hazards_roads_df <- hazards_roads_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
    hazards_roads = sum(physical_access.hazards_roads),
...
security_df <- df %>% 
  select(afg_dist, access_security)

district_security_df <- security_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count = n(),
...
district_women_access_df <- women_access_df %>% group_by(afg_dist) %>% 
  summarise(
    survey_count= n(),
...
accessibility_info <- district_physical_access_df %>% 
  inner_join(district_hazards_roads_df, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_security_df, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_social_df, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(district_women_access_df, by= c("afg_dist"="afg_dist"))
...
accessibility_info$accessibility_score <- accessibility_info$physical_access_issue_score + 
  ...
  accessibility_info$access_security_score + accessibility_info$social_access_issue_score + 
  accessibility_info$women_access_mode

# map score to 25 
# Comment from PV: here again for the factor
accessibility_info$accessibility_index <- accessibility_info$accessibility_score * 1.19047619

# overall MFS: ***************************************
mfs <- district_physical_access_df %>% 
  inner_join(accessibility_info, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(avalibibity_info, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(afordibility_info, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(resilience_info, by= c("afg_dist"="afg_dist")) %>% 
  inner_join(infrastructure_info, by= c("afg_dist"="afg_dist")) %>%
  select(afg_dist, ends_with("index"))

mfs$index <- mfs$avalibibity_index + mfs$afordibility_index + mfs$resilience_index + 
  mfs$infrastructure_index + mfs$accessibility_index

# add all district labels including districts with less than four records: 
district_lables <- df_all %>% select(afg_dist, afg_dist_name) %>% distinct()
mfs <- district_lables %>% left_join(mfs, by= c("afg_dist"="afg_dist"))

# district classification
mfs <- mfs %>% mutate(
  poor_accessibility = if_else(mfs$accessibility_index < 12.5, 1, 0),
  poor_avalibibity = if_else(mfs$avalibibity_index < 15, 1, 0),
  poor_afordibility = if_else(mfs$afordibility_index < 7.5, 1, 0),
  poor_resilience = if_else(mfs$resilience_index < 10, 1, 0),
  poor_infrastructure = if_else(mfs$infrastructure_index < 5, 1, 0),
  
  # number of poor indicators less than 50% of their max scores
  poor_indicators = poor_accessibility + poor_avalibibity + poor_afordibility +
    poor_resilience + poor_infrastructure,

  classification = case_when(
    mfs$index >= 80 & poor_indicators == 0 ~ "Full functionality",
    mfs$index >= 70 | poor_indicators <= 1 ~ "Limited functionality",
    (mfs$index > 25  & mfs$index < 50) | poor_indicators >= 2 ~ "Poor functionality",
    mfs$index < 25 | poor_indicators >= 3 ~ "Severe issues",
    TRUE ~ "Insufficient data"
  )
) %>% select (- starts_with("poor")) %>% arrange(afg_dist)

# round all numeric values:
mfs <- mfs %>%mutate_if(is.numeric, round)

# MFS output as an MS Excel file
mfs_file <- createWorkbook()
addWorksheet(mfs_file, "MFS")
addWorksheet(mfs_file, "avalibibity")
addWorksheet(mfs_file, "afordibility")
...
writeData(mfs_file, sheet = "avalibibity", x = district_items_availability_df)
writeData(mfs_file, sheet = "afordibility", x = afordibility_info)
...
saveWorkbook(mfs_file, paste0("output/MFS.xlsx"), overwrite = T)
