...
availability_info_full <- district_items_availability_df %>% 
  mutate(
    ...
    availability_index = availability_score / ifelse(include_winterization, 1.8, 1.47)  # mapping score to maximum 30 (convert scores to index)
  )
...
food_nfi_item_prices <- c("wheat_local_price",
                          ...
                          "toothpaste_price",
                          "soap_price", # was missed in the draft version and has been added via HQ comment
                          "pen_price",
                          "notebook_price",
                          "rubber_price",
                          "safe_water_price",
...
price_point_modifier <- function(item_median){
  ...
  result = case_when(
    ...
    # {{item_median}} <= national_prices_df[[item]] * 2 ~ -2, wrong
...
district_food_nfi_prices_df <- district_food_nfi_prices_df %>% 
  mutate(
    ...
    soap_point = price_point_modifier(soap_median), # was missed in the draft version and has been added via HQ comment
...
food_nfi_points <- district_food_nfi_prices_df %>%
  mutate(
    # food_nfi_point = rowSums(select(., ends_with("point")) , na.rm = TRUE)  # removed based on the cut of maximum point to 40
...
affordability_info <- financial_info %>% 
  ...
  inner_join(winterization_points, by= c("afg_dist_code"="afg_dist_code")) %>% mutate(
    ...
    ### Comment from PV: I would really suggest to do this as a calculation i.e. not hardcoding the multiplication factor
...
physical_access_df <- df %>% select(afg_dist_code, 
                                     physical_access.hazards_roads, # physical_access.hazards_roads had been selected twice, based on HQ comment, also the mutation part has been corrected by removing physical_access.hazards_roads == 1 |
...
# temp:
write.xlsx(district_hazards_roads_df, "district_hazards_roads_df2.xlsx", overwrite = T)
# end temp
...
mfs <- mfs %>% mutate(
  ...
  # The MFS classification guideline has been updated on 10 April 2023:
  # MFS guideline link: https://acted.sharepoint.com/:w:/s/IMPACTAFG/EdN33HX3oqpPl9z3DyjVnmoBzmxpEn9e4QovoRxoz9ZldA?e=cWPIRH
...
saveWorkbook(mfs_file, paste0("./output/MFS_", format(Sys.time(), "%d_%b_%Y"),".xlsx"), overwrite = T)

# temp:
# number of NAs in each MFS dimensions:
null_availability <- sum(is.na(availability_info_full))
null_accessibility <- sum(is.na(accessibility_info))
null_resilience <- sum(is.na(resilience_info))
null_affordability <- sum(is.na(affordability_info))
null_infrastructure <- sum(is.na(infrastructure_info))
# end temp:
