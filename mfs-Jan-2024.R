...
df_all <- read.csv("./input/AFG2002_JMMI_R43_Jan24_recoded.csv")
# previous_food_basket <- read_xlsx("./input/District_Median_JMMI_R43_Jan24.xlsx", sheet = 3)
# district_codes <- read_xlsx("./input/district_codes.xlsx")
# previous_food_basket <- previous_food_basket %>% inner_join(district_codes, by = c("afg_dist"="district_name"))
# previous_food_basket <- previous_food_basket %>% select(afg_district = district_code, previous_food_basket = food_basket_AFN) 
...
# Conversion coefficients to adjust the MFS score to maximum 100 score.
# -----------------------
# dimension     max score
# -----------------------
# availability:     30
...
CONVERSION.CONFIG <- list(
  # availability:
  availability_coefficient_with_winterization = 1.333, # 1.633
  availability_coefficient_without_winterization = 1.066, # 1.367

  # accessibility:
  accessibility_coefficient_with_winterization = 1.190,

  # resilience:
  resilience_coefficient_with_winterization = 0.430,
  resilience_coefficient_without_winterization = 0.493,

  # affordability:
  # fb if the food basket stability score is available
  # nfb if the food basket stability score is not available
  affordability_coefficient_fb_with_winterization = 1.071,
  affordability_coefficient_fb_without_winterization = 0.789,
  affordability_coefficient_nfb_with_winterization = 0.681,
  affordability_coefficient_nfb_without_winterization = 0.555,

  # infrastructure
  infrastructure_coefficient = 0.555
)
...
AVALABILITY.CONFIG <- list(
  ...
  pulses_widely_available = 2,
  pulses_limited_available = 1,
...
items_availability_df <- items_availability_df %>% mutate(
  wheat_availability_score = case_when(
    items_available_marketplace_wheat == "Widely_available" ~ AVALABILITY.CONFIG$wheat_widely_available,
    items_available_marketplace_wheat == "Limited_availability" ~ AVALABILITY.CONFIG$wheat_limited_available,
    items_available_marketplace_wheat == "Completely_unavailable" ~ 0,
    TRUE ~ NA_real_
  ),
  ...
  pulses_availability_score = case_when(
    items_available_marketplace_pulses == "Widely_available" ~ AVALABILITY.CONFIG$pulses_widely_available,
    items_available_marketplace_pulses == "Limited_availability" ~ AVALABILITY.CONFIG$pulses_limited_available,
    items_available_marketplace_pulses == "Completely_unavailable" ~ 0,
    TRUE ~ NA_real_
  ),
...
district_items_availability_df <- items_availability_df %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
    wheat_availability = getmode(wheat_availability_score),
    rice_availability = getmode(rice_availability_score),
    pulses_availability = getmode(pulses_availability_score),
...
availability_info_full <- district_items_availability_df %>%
  mutate(
    availability_score = case_when(
      include_winterization == TRUE ~ rowSums(.[3:20], na.rm = TRUE),
      TRUE ~ rowSums(.[3:17], na.rm = TRUE)
    ),

    # mapping score to maximum 30 (convert scores to index)
    availability_index = availability_score / ifelse(include_winterization,
      CONVERSION.CONFIG$availability_coefficient_with_winterization,
      CONVERSION.CONFIG$availability_coefficient_without_winterization
    )
  )

availability_info <- availability_info_full %>% select(afg_dist_code, availability_index)

availability_info_full <- availability_info_full %>% mutate_if(~ is.numeric(.), ~ round(., 1))
availability_info <- availability_info %>% mutate_if(~ is.numeric(.), ~ round(., 1))
...
district_food_nfi_prices_df <- df %>%
  select(afg_dist_code, all_of(food_nfi_item_prices)) %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
    wheat_median = median(as.numeric(wheat_price), na.rm = TRUE),
...
national_prices_df <- df %>%
  select(all_of(food_nfi_item_prices), all_of(winterization_item_prices)) %>%
  summarise(
    survey_count = n(),
    wheat_median = median(as.numeric(wheat_price), na.rm = TRUE),
    local_rice_median = median(as.numeric(local_rice_price), na.rm = TRUE),
...
price_point_modifier <- function(item_median) {
  ...
  result <- case_when(
    ...
    {{ item_median }} > national_prices_df[[item]] * 1.5 ~ -2, 
    TRUE ~ NA_real_
  )
  return(result)
}
...
district_food_nfi_prices_df <- district_food_nfi_prices_df %>%
  mutate(
    wheat_point = price_point_modifier(wheat_median),
...
district_winterization_prices_df <- df %>%
  select(afg_dist_code, all_of(winterization_item_prices)) %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
    firewood_median = median(as.numeric(firewood_price), na.rm = TRUE),
...
financial_df <- financial_df %>% mutate(
  financial_access_problem = case_when(
    financial_access.can_not_afford == 1 | financial_access.cannot_pay == 1 | 
      financial_access.expensive_transportation == 1 | financial_access.expensive_fuel == 1 ~ 1,
    TRUE ~ 0
  )
)

# disaggregation the financial_df based on district and calculate financial_access_score for each districts:
financial_info <- financial_df %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
...
affordability_info <- financial_info %>%
  ...
  inner_join(winterization_points, by = c("afg_dist_code" = "afg_dist_code")) %>%
  mutate(

    # updated on 4th May 2023:
    max_score = case_when(
      include_winterization == TRUE ~ 27,
      include_winterization == FALSE ~ 19,
      TRUE ~ 14
    ),
    
    affordability_score = financial_access_score + food_nfi_point + winterization_point,
    ...
    affordability_index = case_when(
      max_score == 22 ~ affordability_final_score * CONVERSION.CONFIG$affordability_coefficient_nfb_with_winterization,
      max_score == 27 ~ affordability_final_score * CONVERSION.CONFIG$affordability_coefficient_nfb_without_winterization,
    )
  )

affordability_info <- affordability_info %>% mutate_if(~ is.numeric(.), ~ round(., 1))
...
district_supply_chain_df <- all_supply_chain_df %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
...
district_one_food_supplier_df <- df %>%
...
district_one_nfi_supplier_df <- df %>%
...
restock_df <- df %>% select(
  afg_dist_code,
  diff_stock_restock_wheat,
...
district_restock_df <- restock_df %>% group_by(afg_dist_code) %>%
  ...
  summarise(
    restock_wheat_median = median(as.numeric(diff_stock_restock_wheat), na.rm = T),
...
district_restock_df <- district_restock_df %>% mutate(
  restock_wheat_score = restock_score(restock_wheat_median),
...
district_restock_df2 <- district_restock_df %>%
  mutate(
...
winterization_restock_df <- df %>% select(
  afg_dist_code,
...
district_winterization_restock_df <- winterization_restock_df %>%
  group_by(afg_dist_code) %>%
  summarise(
...
# assigning sum up scores for each district:
district_winterization_restock_df2 <- district_winterization_restock_df %>%
  mutate(
...
resilience_info <- resilience_info %>% mutate(
  ...
  resilience_index = case_when(
    include_winterization == TRUE ~ resilience_score * CONVERSION.CONFIG$resilience_coefficient_with_winterization,
    TRUE ~ resilience_score * CONVERSION.CONFIG$resilience_coefficient_without_winterization
  )
)

resilience_info <- resilience_info %>% mutate_if(~ is.numeric(.), ~ round(., 1))
...
district_damaged_facilities_df <- damaged_facilities_df %>%
  ...
  mutate(
    damaged_facilities_score = case_when(
      ...
      damaged_facilities_percentage > 50 ~ 0, # maybe un-necessary
      TRUE ~ 0
    )
  ) %>%
...
financial_services_df <- df %>% select(
  afg_dist_code,
...
district_financial_services_df <- financial_services_df %>%
  group_by(afg_dist_code) %>%
  summarise(
    n(),
  ...
  mutate(
    fsp_types_count = sum(
      banks, mobile_money_agents, formal_transfer_services,
      hawala, microfinance_institutions, village_banks,
      credit_unions, local_businesses, members_community
    )
  )

district_financial_services_df2 <- district_financial_services_df %>%
  mutate(
...
payment_df <- df %>%
  ...
  mutate(
    modality = sum(
      modality.mobile_money, modality.credit,
...
infrastructure_info$infrastructure_index <- infrastructure_info$infrastructure_score * CONVERSION.CONFIG$infrastructure_coefficient

infrastructure_info <- infrastructure_info %>% mutate_if(~ is.numeric(.), ~ round(., 1))
  ...
  mutate(
    physical_access_issue = case_when(
      (physical_access.hazardous_building == 1 |
        ...
        physical_access.distance_market == 1 ) & physical_access.hazards_roads == 0 ~ 1,
      TRUE ~ 0
    )
  ) %>%
...
district_hazards_roads_df <- hazards_roads_df %>%
  ...
  mutate(
    hazards_roads_score = case_when(
      ...
      hazards_roads_percentage > 50 ~ 0, # maybe un-necessary
      TRUE ~ 0
    )
  )
...
district_women_access_df <- women_access_df %>%
  group_by(afg_dist_code) %>%
  summarise(
...
# map score to 25
accessibility_info$accessibility_index <- accessibility_info$accessibility_score * CONVERSION.CONFIG$accessibility_coefficient_with_winterization

accessibility_info <- accessibility_info %>% mutate_if(~ is.numeric(.), ~ round(., 1))
...
mfs <- mfs %>%
  ...
  select(
    afg_dist_code, afg_dist, availability_index, accessibility_index, resilience_index,
    affordability_index, infrastructure_index, MFS, classification, sorted
  ) %>%
  arrange(desc(MFS)) %>%
  arrange(desc(sorted)) %>%
  select(-sorted)
...
colnames(mfs)[colnames(mfs) == "availability_index"] <- "availability [30]"
colnames(mfs)[colnames(mfs) == "accessibility_index"] <- "accessibility [25]"
colnames(mfs)[colnames(mfs) == "resilience_index"] <- "resilience [20]"
colnames(mfs)[colnames(mfs) == "affordability_index"] <- "affordability [15]"
colnames(mfs)[colnames(mfs) == "infrastructure_index"] <- "infrastructure [10]"
