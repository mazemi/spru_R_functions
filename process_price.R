...
process_jmmi_prices <- function(tool_path){
  ...
  for (file in file_names) {
    ...
    # extract level value
    path_parts <- unlist(strsplit(file, "/"))
    relevant_part <- path_parts[4]
    extracted_level <- sub("_.*", "", relevant_part)
    
    cat(paste0("processing ",extracted_level, " prices"), "\n")
    data <- list()
  ...
  cat("longitudinal prices has been generated.", "\n")
}
