...
# import recoded data:
df_all <- read.csv("./input/AFG2002_JMMI_R39_SEP23_recoded.csv")
...
previous_food_basket <- read_xlsx("./input/District_Median_JMMI_R38_AUG23.xlsx", sheet = 3) %>%
...
CONVERSION.CONFIG <- list(
  # availability:
  availability_coefficient_with_winterization = 1.633,
  availability_coefficient_without_winterization = 1.367,
...
district_items_availability_df <- district_items_availability_df %>%
  rowwise() %>%
...
indebtedness_info <- indebtedness_df %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
...
food_basket_df <- df %>% select(
  afg_dist_code, wheat_imported_calculation, rice_price_calculation,
...
food_basket_district_df <- food_basket_df %>%
  group_by(afg_dist_code) %>%
  summarise(
    survey_count = n(),
...
affordability_info <- financial_info %>%
  ...
  mutate(
    ...
    affordability_index = case_when(
      max_score == 14 ~ affordability_final_score * CONVERSION.CONFIG$affordability_coefficient_fb_with_winterization,
      max_score == 19 ~ affordability_final_score * CONVERSION.CONFIG$affordability_coefficient_fb_without_winterization,
...
# red style if the the dimension is less than half 
...
setColWidths(mfs_file, sheet = 1, cols = 1:9, widths = 20)
...
saveWorkbook(mfs_file, paste0("./output/MFS4_", JMMI_round, "_", format(Sys.time(), "%d_%b_%Y"), ".xlsx"), overwrite = T)
